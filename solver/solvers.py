...
def mrd_parallel(word, possible_words):
    bucket_squared_sum = 0
    buckets = get_word_buckets(word, possible_words)
    for bucket in buckets:
        if bucket != []:
            bucket_squared_sum += len(bucket)*len(bucket)
    return bucket_squared_sum
...
def most_rapid_decrease_guess(allowed_words, possible_words, priors):
    ...
    bucket_sums = Parallel(n_jobs=4)(delayed(mrd_parallel)(word, possible_words) for word in allowed_words)
...
def gep_parallel_naive(word, possible_words):
    ...
    return np.sum(ans_probabilities)
